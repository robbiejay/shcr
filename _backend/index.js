...
const options = {
  key: fs.readFileSync('/usr/local/nginx/html/wp_api/key.pem'),
  cert: fs.readFileSync('/usr/local/nginx/html/wp_api/cert.pem')
};
...
function getShows(page_num) {
  ...
  //  console.log('Started downloading page ' + page_num + '...')
    ...
    //  console.log(JSON.parse(savedFile))
       ...
       fs.writeFileSync('/usr/local/nginx/html/wp_api/data/_temp/shows_' + page_num + '.json', jsonPageData);
    ...
    //   process.stdout.write('page ' + page_num + ' written to json!')
       ...
       fs.writeFileSync('/usr/local/nginx/html/wp_api/data/shows/total_pages.json', jsonTotalPages);
...
function getTags(tag_num) {
  http.get('https://admin.hkcr.live/wp-json/wp/v2/tags/' + tagMasterList[tag_num], (resp) => {
    ...
    //    process.stdout.write('downloading ' + tag_num + ' of ' + tagMasterList.length);
        ...
        fs.writeFileSync('/usr/local/nginx/html/wp_api/tag_id_list.json', tagMasterList);
        fs.writeFileSync('/usr/local/nginx/html/wp_api/tag_name_list.json', tagNameList);
...
let fileURI = '/usr/local/nginx/html/wp_api/data/_temp/shows_' + real_index + '.json';
fs.readFile(fileURI, (err, file) => {
  ...
  rawdata.forEach(block => {
    ...
    masterArr[tagIndex][tagName].push(block);
      ...
      fs.writeFileSync('/usr/local/nginx/html/wp_api/data/shows/single/' + url + '.json', singleJsonData)
  })
  newFile = JSON.stringify(rawdata);
  newFileURI = '/usr/local/nginx/html/wp_api/data/shows/shows_' + real_index + '.json'
...
//  process.stdout.write('shows_' + real_index + '.json has been edited!')
    ...
    masterArr.forEach(item => {
      ...
      //  console.log(item[prop].length);
       ...
       if(((index + 1) % 9 == 0) || (index + 1 == item[prop].length)) {
          ...
          fs.writeFileSync('/usr/local/nginx/html/wp_api/data/shows/genres/_totals/' + prop.replace(/ /g, "_").toLowerCase() + '.json', jsonIndex)
          fs.writeFileSync('/usr/local/nginx/html/wp_api/data/shows/genres/' + prop.replace(/ /g, "_").toLowerCase() + '_' + pageNumber + '.json', fileArrJson);
  ...
  //        process.stdout.write(prop + '_' + pageNumber + '.json has been created')
    ...
    fs.writeFileSync('/usr/local/nginx/html/wp_api/data/shows/genres/master.json', jsonObj);
...
function getSchedule() {
  http.get('https://admin.hkcr.live/wp-json/wp/v2/posts?_embed&categories=15', (resp) => {
    ...
    resp.on('end',() => {
      ...
      fs.writeFileSync('/usr/local/nginx/html/wp_api/data/schedule/schedule.json', jsonData);
...
function getHighlights() {
  http.get('https://admin.hkcr.live/wp-json/wp/v2/posts?_embed&categories=164', (resp) => {
    ...
    resp.on('end',() => {
      let formattedData = JSON.parse(data);
      let jsonData = JSON.stringify(formattedData);
      fs.writeFileSync('/usr/local/nginx/html/wp_api/data/highlights/highlight_reel.json', jsonData);
  ...
  http.get('https://admin.hkcr.live/wp-json/wp/v2/posts?_embed&categories=177&per_page=100', (resp) => {
    ...
    resp.on('end', () => {
      ...
      formattedData.forEach(item => {
        ...
        fs.writeFileSync('/usr/local/nginx/html/wp_api/data/residents/single/' + url + '.json', singleJsonData)
      })
      let jsonData = JSON.stringify(formattedData);
      fs.writeFileSync('/usr/local/nginx/html/wp_api/data/residents/residents.json', jsonData);
...
function getUpcomingShows() {
  http.get('https://admin.hkcr.live/wp-json/wp/v2/posts?_embed&categories=178&per_page=24', (resp) => {
    ...
    resp.on('end', () => {
      ...
      // }
        fs.writeFileSync('/usr/local/nginx/html/wp_api/data/upcoming-shows/single/' + url + '.json', singleJsonData)
      })
      let jsonData = JSON.stringify(formattedData);
      fs.writeFileSync('/usr/local/nginx/html/wp_api/data/upcoming-shows/upcoming_shows.json', jsonData);
  ...
  http.get('https://admin.hkcr.live/wp-json/wp/v2/posts?_embed&categories=179', (resp) => {
    ...
    resp.on('end', () => {
      ...
      formattedData.forEach(item => {
        let url = item.title.rendered.replace(/[^a-zA-Z0-9]+/g, "-").toLowerCase()
        let singleJsonData = JSON.stringify(item);
        fs.writeFileSync('/usr/local/nginx/html/wp_api/data/blog/single/' + url + '.json', singleJsonData)
      })
      let jsonData = JSON.stringify(formattedData);
      fs.writeFileSync('/usr/local/nginx/html/wp_api/data/blog/blog.json', jsonData);
...
function setUpdatedTime() {
  ...
  fs.writeFileSync('/usr/local/nginx/html/wp_api/data/timestamp.json', date)
  console.log('Timestamp updated!');
  console.log('All updates completed, exiting script');
  process.exit();
}
