...
export class UpcomingGuardService implements CanActivate {
...
return this.postsService.getSingleUpcoming(title.replace(/[^a-zA-Z0-9]+/g, "-").toLowerCase()).pipe(
  map(
    showData => {
      ...
      this._meta.updateTag({ name: 'description', content: showData.content.rendered});
  //    this._meta.updateTag({ name: 'keywords', content: showData.title + ', mix, ' + JSON.stringify(showData.tags)},"name='keywords'");
      ...
      this._meta.updateTag({ property: 'og:description', content: showData.content.rendered});
           return true;
    }
  )
)
